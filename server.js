...
// ------------------------------
app.get("/ops-import-clients", requireAdminToken, async (req, res) => {
  try {
    const { data, error } = await supabaseAdmin
      .from("admin_users")
      .select("business_id, brand_name, email, google_import_enabled, active, plan_tier")
      .eq("plan_tier", PLAN_PREMIUM)
      .eq("google_import_enabled", true)
      .eq("active", true)
      .order("business_id", { ascending: true });
    ...
    console.error("GET /ops-import-clients error:", err);
    res.status(500).send("Server error");
  }
});

app.post("/ops-import-google-review", requireAdminToken, async (req, res) => {
  try {
    const businessId = (req.body.businessId || "").trim();
    ...
    if (!profile.google_import_enabled) {
      return res.status(403).send("Google review import is disabled for this client.");
    }
    ...
    console.error("POST /ops-import-google-review error:", err);
    res.status(500).send(err.message || "Server error");
  }
});
...
// ------------------------------
// Google import (client-side route retained)
// ------------------------------
