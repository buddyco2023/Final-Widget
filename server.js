...
</script>  <script src="${PUBLIC_APP_URL}/widget.js" defer></script>`;  }
...
if (widgetEnabled) {
text +=   Website Review Page URL:   ${data.reviewPageUrl || "(not provided)"}  ;
}

if (hostedEnabled) {
text +=   Hosted Review Page:   ${PUBLIC_APP_URL}/r/${data.businessId}  ;
}
...
.from("admin_users")
.select(  email,   business_id,   must_reset_password,   notifications_enabled,   notification_email,   plan_tier,   google_import_enabled,   branding_enabled,   review_page_url,   brand_name,   brand_primary,   brand_secondary,   brand_logo_url,   active,   setup_fee_paid,   partner_name,   partner_email,   delivery_type,   hosted_header,   hosted_footer,   base_monthly_price,   hosted_addon_price,   total_monthly_price,   setup_fee_amount,   billing_day,   next_invoice_date,   last_payment_date,   billing_status,   partner_commission_percent,   terms_accepted,   terms_accepted_at  )
.eq("email", email)
.limit(1);
...
const { data, error } = await supabaseAdmin
.from("admin_users")
.select(  business_id,   email,   plan_tier,   active,   brand_name,   brand_primary,   brand_secondary,   brand_logo_url,   branding_enabled,   review_page_url,   notifications_enabled,   notification_email,   google_import_enabled,   delivery_type,   hosted_header,   hosted_footer,   base_monthly_price,   hosted_addon_price,   total_monthly_price,   setup_fee_amount,   billing_day,   next_invoice_date,   last_payment_date,   billing_status,   partner_name,   partner_email,   partner_commission_percent  )
...
const { data, error } = await supabaseAdmin
.from("admin_users")
.select(  business_id,   email,   plan_tier,   delivery_type,   brand_name,   active,   base_monthly_price,   hosted_addon_price,   total_monthly_price,   setup_fee_amount,   billing_day,   next_invoice_date,   last_payment_date,   billing_status,   partner_name,   partner_email,   partner_commission_percent  )
